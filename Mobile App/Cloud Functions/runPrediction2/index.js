...
function cmlePredict(b64img) {
    return new Promise((resolve, reject) => {
        google.auth.getApplicationDefault(function (err, authClient) {
            ...
            const params = {
                auth: authClient,
                name: 'projects/bouquetdetectionproject/models/bouquetDetection2',  
                resource: {
                    instances: [
                    {
                        "inputs": {
...
exports.runPrediction2 = functions.storage.object().onFinalize((object, context) => {
...
//     const object = event.data;
    ...
    if (filePath.startsWith('images/')) {  
        ...
        .then((results) => {
            ...
            let outlinedImgPath = '';
            let imageRef = db.collection('predicted_images2').doc(filePath.slice(7));
            ...
            if (uploadFlag == true) {
                outlinedImgPath = `outlined_img2/${filePath.slice(7)}`;
                imageRef.set({
